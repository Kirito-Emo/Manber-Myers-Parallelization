...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        if (argc != 2)
        {
            std::cerr << "Usage: " << argv[0] << " <target-MB>\n";
    ...
    // Broadcast target_mb to all ranks
    MPI_Bcast(&target_mb, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // Open file once to get actual size
    ...
    const size_t n = static_cast<size_t>(fsize.tellg()); // actual string size in bytes
    fsize.close();
