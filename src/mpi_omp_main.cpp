...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        auto [ptr, ec] = std::from_chars(argv[1], argv[1] + std::strlen(argv[1]), target_mb);
        if (ec != std::errc{} || *ptr != '\0' ||
            !(target_mb == 1 || target_mb == 50 || target_mb == 100 || target_mb == 200 || target_mb == 500))
        {
    ...
    MPI_Bcast(&target_mb, 1, MPI_INT, 0, MPI_COMM_WORLD);

    // Determine actual file size on rank 0
    size_t n = 0;
    ...
    // Record OpenMP threads per rank (max)
    ...
    // Build suffix array - the subset builder uses OpenMP internally
    ...
    std::vector<int> all_sa(total_sa); // meaningful on rank 0
    ...
    MPI_Gather(&omp_threads, 1, MPI_INT, threads.data(), 1, MPI_INT, 0, MPI_COMM_WORLD);
    ...
    if (rank == 0)
    {
        ...
        for (int r = 0; r < size; ++r)
        {
            std::cout << "rank=" << r << " threads=" << threads[r] << " time_sa_local=" << std::fixed
                      << std::setprecision(6) << times_sa[r] << " time_comm=" << times_comm[r]
                      << " time_io=" << times_io[r] << " time_alloc=" << times_alloc[r] << "\n";
        }

        // Load full text (rank 0, excluded from compute)
        ...
        // Aggregates (max across ranks = bottleneck)
        double time_sa_max = 0.0, time_alloc_max = 0.0, time_io_max = 0.0, time_comm_max = 0.0;
        for (int r = 0; r < size; ++r)
        {
            time_sa_max = std::max(time_sa_max, times_sa[r]);
            time_alloc_max = std::max(time_alloc_max, times_alloc[r]);
            time_io_max = std::max(time_io_max, times_io[r]);
            time_comm_max = std::max(time_comm_max, times_comm[r]);
        }
        ...
        const double T_seq = load_seq_baseline_for_mb(target_mb, rank);
        const bool have_baseline = (T_seq > 0.0);
        const double speedup = (have_baseline && time_compute_pure > 0) ? (T_seq / time_compute_pure) : 0.0;
        const double efficiency = (have_baseline && size > 0) ? (speedup / size) : 0.0;

        // Report
        std::cout << std::fixed << std::setprecision(6);
        std::cout << "=== Suffix-Array + LCP (MPI+OpenMP) ===\n";
        std::cout << "target_size=" << target_mb << " MB (string=" << mb_from_bytes(n) << " MB)" << "   ranks=" << size
                  << "\n";
        ...
        std::cout << "time_transfers_comm=" << time_comm_max << " s\n";
